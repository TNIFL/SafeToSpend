...
### 보안 관련 환경변수(권장)
- 외부에서 접근 가능한 서버는 기본 `SECRET_KEY`로 시작하면 안 됩니다.
- 기본 `SECRET_KEY`는 `APP_ENV=development|dev|local|test` 이고 localhost 전용 실행일 때만 허용됩니다.
- `APP_ENV`가 비어 있거나 외부 bind 환경이면 기본 `SECRET_KEY`로 앱이 바로 실패합니다.
- `ALLOW_INSECURE_DEV_SECRET_KEY=1` 은 localhost 개발 프로세스 예외 판단에만 쓰이며, 외부 bind를 허용하지 않습니다.
- 프록시 환경에서만 XFF 신뢰:
  - `TRUST_PROXY_X_FORWARDED_FOR=1`
- 레이트리밋 Redis 백엔드(선택):
  - `RATE_LIMIT_REDIS_URL=redis://localhost:6379/0`

## 3-1) 운영 안전 체크(권한/탈퇴/설정/코드 전달)
- 관리자 권한 부여:
  - 회원가입만으로 관리자가 되지 않습니다.
  - 명시적 부여만 허용:
    - `flask --app app set-admin-role --email admin@example.com --grant`
- 관리자 권한 회수:
  - `flask --app app set-admin-role --email admin@example.com --revoke`
- 계정 삭제 검증:
  - 탈퇴 후 `official_data_documents`, `nhis_*`, `billing_*`, `evidence_items`, `receipt_items`가 해당 `user_pk` 기준으로 남아 있지 않은지 확인합니다.
  - 파일 삭제 오류는 로그로 남을 수 있지만 DB purge 자체는 성공해야 합니다.
- 코드 전달용 ZIP:
  - 사전 점검:
  ...
  - `uploads/**`, `reports/**`, `*.dump`, 캐시, 실제 `.env*`, `.venv/**` 가 포함되면 안 됩니다. (`.env.example`은 예외)
...
- 관리자 접근:
  - 일반 계정 생성 후 명시적으로 관리자 권한 부여
    - `flask --app app set-admin-role --email <대상 이메일> --grant`
  - 부여된 계정으로 로그인
...
- 설정 누락:
  - 관리자 권한이 없는 계정은 이메일 문자열과 무관하게 `/admin/inquiries` 접근이 차단되는지 확인
