...
- jpg / jpeg / png / webp / heic 이미지를 고른 뒤 `파싱 시작`을 누르면 파일명.형식 목록이 먼저 보이는지
- 파싱 단계에서 이미지 이름 목록이 스크롤 가능하고, 항목을 누르면 매장 명 / 날짜 및 시간 / 결제 금액 / 결제 항목 / 결제 카드 및 계좌번호 / 메모가 보이는지
- 파싱 중에는 스피너, 완료 후에는 체크 표시가 보이고, 완료 알림 토스트를 눌렀을 때 결과 확인 단계로 이동하는지
- 50개 초과 선택 시 바로 막히는지
- 계좌가 있으면 선택 드롭다운이 보이고, 없으면 계좌 미지정으로 진행되는지
- 결과 확인 단계에서 거래 생성 후 증빙 보관함에서 첨부 파일이 보이는지
