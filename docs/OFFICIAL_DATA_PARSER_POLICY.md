...
registry / parser / upload의 책임은 분리합니다.

- registry: 문서 타입 후보 식별
- parser: 핵심 값 추출
- upload: `parsed / needs_review / unsupported / failed` 상태 고정

tabular 문서는 첫 행만 보지 않고 앞 1~3행 안에서 가장 그럴듯한 헤더 행을 찾습니다.
PDF 문서는 제목/발급기관/핵심 라벨의 소폭 공백 차이까지 허용합니다.
핵심 구조가 모자라면 `supported_document_type`로 억지 승격하지 않고 `needs_review` 또는 `unsupported`로 닫습니다.
...
## 10. 이번 보정에서도 계속 막는 변형

- 기관명과 문서명 둘 다 불명확한 문서
- 핵심 금액/날짜/세목 없이 납부내역처럼 보이는 문서
- 안내문/주석만 있고 실제 데이터 구조가 없는 문서
- NHIS 자격 자료에서 기준일/가입자 유형/자격 상태가 빠진 문서
- 전혀 다른 문서를 기존 document_type으로 오인하게 만드는 과한 alias 완화

## 11. explainability 원칙
